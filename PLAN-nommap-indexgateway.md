...
- Phase 1 (buffered): shipped. Not the production path — buffered holds every
  cached file's bytes on the Go heap and OOMed pods in loki-dev-005 (see
  2026-07-29 entry).
...
- **2026-07-29**: heavy perf iteration day. Started from a buffered-mode
  OOM report (loki-dev-005), diagnosed via CPU + goroutine profiling, and
  ...
  measured in preprod against `loki-dev-005/index-gateway`.
  ...
  **Two production incidents examined:**
  ...
  - *loki-dev-005 buffered-mode OOM* (initial trigger for the day):
  ...
  - *loki-dev-002 `--analyze-labels` incident (~13:33–14:17 UTC)*: user
    ran `logcli series '{}' --analyze-labels --since=72h` on tenant 157702.
